...
alias tools.docker-clean-all='echo "Deleting exited containers..." && (docker rm -v $(docker ps -a -q -f status=exited) || true) && echo "Deleting untagged images." && docker rmi $(docker images -f "dangling=true" -q) && echo "SUCCESS."'
